...
import { CheckCircle, Circle, ArrowRight, User, Settings, Briefcase, Zap, Calendar, MessageSquare, Video, Phone, Users, XCircle, X } from "lucide-react";
...
const FUNNEL_TEMPLATES = [
  {
    ...
    features: ["Round Robin scheduling", "Auto-warmup emails", "AI pain-point tagging in CRM"],
  ...
  },
  {
    ...
    features: ["SLA Budget Filter (re-routes to webinar)", "Pre-loaded Meeting Room presentations"],
  ...
  },
  {
    ...
    features: ["AI Email Agent (objections & booking)", "Automated CRM stage triggers"],
  ...
  },
  {
    ...
    features: ["Attention tracking (WhatsApp reminders)", "AI warm-lead routing"],
  ...
  },
  {
    ...
    features: ["Strict 5-minute SLA timer", "Re-routes to AI voice bot if SLA missed"],
  ...
  },
  {
    ...
    features: ["Tailored architecture", "Dedicated solutions engineer"],
...
export function Step5PipelineFunnels() {
  ...
  const handleSelection = (template: typeof FUNNEL_TEMPLATES[0], type: 'diy' | 'service' | 'hire') => {
    const isAlreadySelected = 
      (type === 'diy' && state.clientProvided.includes(template.id)) ||
      (type === 'service' && state.cartItems.some(i => i.optionId === template.service.id)) ||
      (type === 'hire' && state.cartItems.some(i => i.optionId === template.hire.id));
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12">
        {FUNNEL_TEMPLATES.map((template) => {
          ...
          return (
            <div 
              key={template.id}
              id={`diy-item-${template.id}`}
              onClick={() => handleSelectTemplate(template.id)}
              className={`cursor-pointer group relative bg-white dark:bg-slate-900/50 backdrop-blur-md rounded-3xl p-8 transition-all duration-300 ${
                isSelected 
            ...
            >
              <div className="flex items-start justify-between mb-6">
                <div>
                  <h3 className={`text-xl font-bold transition-colors ${isSelected ? 'text-indigo-900 dark:text-indigo-100' : 'text-slate-900 dark:text-white'}`}>
                    {template.name}
                  </h3>
                  <span className={`inline-block px-3 py-1.5 mt-2 text-xs font-bold rounded-lg uppercase tracking-wider transition-colors ${isSelected ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'}`}>
                    {template.tagline}
                  </span>
                </div>
                {isSelected ? (
                  <div 
                    ...
                    className="relative w-7 h-7 flex items-center justify-center text-red-500 transition-colors cursor-pointer"
                  >
                    <CheckCircle className="w-7 h-7 text-indigo-600 dark:text-indigo-400 absolute opacity-0 md:opacity-100 md:group-hover:opacity-0 transition-opacity" />
                    <XCircle className="w-7 h-7 text-red-500 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity absolute" />
                  </div>
                ) : (
                  <Circle className="w-7 h-7 text-slate-300 dark:text-slate-600 group-hover:text-indigo-300 dark:group-hover:text-indigo-500/50 transition-colors" />
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2.5 mb-8">
                {template.flow.map((step, idx) => (
                  <React.Fragment key={idx}>
                    <div className={`flex items-center gap-2 px-3 py-2 rounded-xl border transition-colors ${isSelected ? 'bg-indigo-50 border-indigo-100 dark:bg-indigo-500/10 dark:border-indigo-500/20' : 'bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700'}`}>
                      <span className={`${isSelected ? 'text-indigo-500' : 'text-slate-400'}`}>{step.icon}</span>
                      <span className={`text-xs font-bold ${isSelected ? 'text-indigo-900 dark:text-indigo-200' : 'text-slate-600 dark:text-slate-300'}`}>{step.label}</span>
                    </div>
                    {idx < template.flow.length - 1 && (
                      <ArrowRight className={`w-4 h-4 ${isSelected ? 'text-indigo-300 dark:text-indigo-500/50' : 'text-slate-300 dark:text-slate-600'}`} />
                    )}
                  </React.Fragment>
                ))}
              </div>
              ...
              <div className="space-y-3">
                <p className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Key Features</p>
                <ul className="space-y-2">
                  {template.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3 text-sm font-medium text-slate-600 dark:text-slate-400">
                      <div className={`w-1.5 h-1.5 rounded-full mt-1.5 shrink-0 transition-colors ${isSelected ? 'bg-indigo-500' : 'bg-slate-300 dark:bg-slate-600'}`} />
      ...
      {selectedTemplate && (
        <div className="bg-slate-50 dark:bg-slate-900/30 border-2 border-indigo-100 dark:border-indigo-500/20 rounded-2xl p-6 shadow-sm animate-in slide-in-from-bottom-8 fade-in zoom-in-95 duration-500">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            ...
            <div className="flex flex-col gap-2 shrink-0 w-full md:w-[280px]">
              ...
              <button
                ...
                className={`w-full relative flex items-center justify-between p-3 rounded-xl border-2 transition-all duration-200 ${
                  state.clientProvided.includes(selectedTemplate.id)
                    ...
                    : 'border-white dark:border-slate-800 bg-white dark:bg-slate-800 hover:border-emerald-200 dark:hover:border-emerald-500/30 hover:bg-emerald-50/30 dark:hover:bg-emerald-500/5 shadow-sm cursor-pointer hover:-translate-x-1'
                }`}
              >
                <div className="flex items-center gap-3">
                  <Settings className={`w-5 h-5 transition-colors ${state.clientProvided.includes(selectedTemplate.id) ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-400'}`} />
                  <span className={`font-bold text-sm transition-colors ${state.clientProvided.includes(selectedTemplate.id) ? 'text-emerald-700 dark:text-emerald-400' : 'text-slate-700 dark:text-slate-300'}`}>
                    Set it up myself
                  </span>
                </div>
                <div className="flex items-center justify-end w-8 h-8 relative shrink-0 group">
                  <span className={`text-xs text-slate-500 font-medium transition-opacity absolute right-0 ${state.clientProvided.includes(selectedTemplate.id) ? 'opacity-0 md:opacity-100 md:group-hover:opacity-0' : ''}`}>$0</span>
                  {state.clientProvided.includes(selectedTemplate.id) && (
                    <div 
                      onClick={(e) => {
                        e.stopPropagation();
              ...
              <button
                ...
                className={`w-full relative flex items-center justify-between p-3 rounded-xl border-2 transition-all duration-200 ${
                  state.cartItems.some(i => i.optionId === selectedTemplate.service.id)
                    ...
                    : 'border-white dark:border-slate-800 bg-white dark:bg-slate-800 hover:border-indigo-200 dark:hover:border-indigo-500/30 hover:bg-indigo-50/30 dark:hover:bg-indigo-500/5 shadow-sm cursor-pointer hover:-translate-x-1'
                }`}
              >
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center justify-end w-16 h-8 relative shrink-0 group">
                  <span className={`text-xs font-mono font-semibold text-slate-500 transition-opacity absolute right-0 ${state.cartItems.some(i => i.optionId === selectedTemplate.service.id) ? 'opacity-0 md:opacity-100 md:group-hover:opacity-0' : ''}`}>+${selectedTemplate.service.price.toLocaleString()}</span>
                  {state.cartItems.some(i => i.optionId === selectedTemplate.service.id) && (
                    <div 
                      onClick={(e) => {
                        e.stopPropagation();
              ...
              >
                ...
                <div className="flex items-center justify-end w-16 h-8 relative shrink-0 group">
                  <span className={`text-xs font-mono font-semibold text-slate-500 transition-opacity absolute right-0 ${state.cartItems.some(i => i.optionId === selectedTemplate.hire.id) ? 'opacity-0 md:opacity-100 md:group-hover:opacity-0' : ''}`}>+${selectedTemplate.hire.price.toLocaleString()}</span>
                  {state.cartItems.some(i => i.optionId === selectedTemplate.hire.id) && (
                    <div 
                      onClick={(e) => {
                        e.stopPropagation();
